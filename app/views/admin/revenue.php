...
<body>
    ...
    display: flex;">
								<h4 class="card-title float-left mt-2">Doanh thu khách sạn <?php echo $data["hotel"][0]["hotel_name"];?></h4>
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="card card-table">
                            ...
                            <?php
                                $revenue_hotel_year=0;
                                for($i=0;$i<count($data["room"]);$i++)
                                {
                                    $this->Booking= new BookingModel;
                                    $revenue_room_year=$this->Booking->getRevenueRoomYear($data['room'][$i]['room_id'],$_GET['year']);
                                    echo "<p>".$data['room'][$i]['room_name']." : ".number_format($revenue_room_year['total_revenue'], 0, ',', '.')." VND</p>";
                                    $revenue_hotel_year=$revenue_hotel_year+$revenue_room_year['total_revenue'];
                                }
                                echo "<p>Tổng doanh thu : ".number_format($revenue_hotel_year, 0, ',', '.') . ' VND'."</p>";
                            ?>
                            <p id="result"></p>
        ...
        selectElement.addEventListener("change", function() {
            ...
            if (selectedYear) {
                ...
                if (currentUrl.includes("?")) {
                    ...
                    newUrl = currentUrl.split("?")[0] + "?adminHotel_id=<?php echo $_GET["adminHotel_id"];?>&&year=" + encodeURIComponent(newValue);
                } else {
                    // Nếu URL chưa có query string
                    newUrl = currentUrl + "?adminHotel_id=<?php echo $_GET["adminHotel_id"];?>&&year=" + encodeURIComponent(newValue);
                }
